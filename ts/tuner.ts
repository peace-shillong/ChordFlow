...
export class TunerController {
  ...
  private inTuneConsecutiveHits: number = 0;
  private isAutoAdvancing: boolean = false;

  // DOM elements
  private modalEl: HTMLElement | null = null;
  private micPromptEl: HTMLElement | null = null;
  private instPillsContainerEl: HTMLElement | null = null;
  private stringPillsContainerEl: HTMLElement | null = null;
  ...
  public renderInstrumentPills(): void {
    if (!this.instPillsContainerEl) return;
  ...
  public renderStringPills(): void {
    if (!this.stringPillsContainerEl) return;
  ...
  public async startListening(): Promise<void> {
    ...
    try {
      ...
      if (this.statusEl) this.statusEl.textContent = "Mic access blocked";
    }
  }
      ...
      if (absCents <= 4) {
        ...
        if (this.inTuneConsecutiveHits >= 24 && !this.isAutoAdvancing) {
          ...
          setTimeout(() => {
            ...
            this.inTuneConsecutiveHits = 0;
            if (this.statusEl) this.statusEl.textContent = "Ready for next string";
          }, 900);
        }
      } else if (absCents <= 15) {
