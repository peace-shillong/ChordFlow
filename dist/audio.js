...
export class AudioEngine {
     ...
     */
    playNote(midi, instrument = "piano", startTime, duration = 1.2) {
     ...
     */
    playChord(midiNotes, instrument = "guitar", startTime, isStrummed = true) {
        const ctx = this.ensureContext();
        const t = startTime !== undefined ? startTime : ctx.currentTime;
    ...
    // --- Synthesis Models ---
    synthesizePiano(ctx, freq, t, duration) {
        if (!this.masterGain)
            return;
    ...
    synthesizeGuitar(ctx, freq, t, duration) {
        if (!this.masterGain)
            return;
    ...
    synthesizeUkulele(ctx, freq, t, duration) {
        if (!this.masterGain)
            return;
    ...
    synthesizeHarmonica(ctx, freq, t, duration) {
        if (!this.masterGain)
            return;
